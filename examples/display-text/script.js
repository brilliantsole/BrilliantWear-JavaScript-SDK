...
const selectFont = async (newFontName) => {
  ...
  if (newFontName.startsWith("Noto Sans")) {
    if (newFontName.includes("JP")) {
      targetLanguage = "Japanese";
    } else if (newFontName.includes("KR")) {
...
const createTranslator = async () => {
  ...
  console.log("created translator", translator);
};

const translator_progress_callback = (x) => {
...
const translate = async (string) => {
  ...
  const result = await translator(string, {
    ...
    tgt_lang: LANGUAGES[targetLanguage],
  });
